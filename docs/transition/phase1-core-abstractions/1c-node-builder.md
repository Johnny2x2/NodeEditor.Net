# 1C — Node Builder (INodeBuilder, NodeBuilder)

> **Parallelism**: Depends on **1A** + **1B** completing first. Then unlocks Phase 2 and Phase 4.

## Prerequisites
- **1A** — Uses `ExecutionSocket.TypeName` and `StreamSocketInfo`
- **1B** — Uses `NodeBase` (for `Configure(INodeBuilder)` contract)

## Can run in parallel with
- Nothing within Phase 1 (this is the last piece). Unlocks all subsequent phases.

## Deliverables

### `INodeBuilder` — Fluent builder interface
...
public sealed class NodeBuilder : INodeBuilder
{
    ...
    public NodeDefinition Build()
    {
        ...
        return new NodeDefinition(
            Id: id, Name: name, Category: _category, Description: _description,
            Inputs: inputsSnapshot, Outputs: outputsSnapshot,
            Factory: () => new NodeData(
                Id: Guid.NewGuid().ToString("N"), Name: name,
                Callable: callable, ExecInit: execInit,
                Inputs: inputsSnapshot, Outputs: outputsSnapshot, DefinitionId: id),
            NodeType: NodeType,
    ...
    {
        if (NodeType is not null) return $"{NodeType.FullName}";
        return _name;
    }
...
- [ ] Depends on Phase 2A for `NodeDefinition` to be extended with optional params
