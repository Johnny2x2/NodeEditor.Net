# 3B — ExecutionRuntime & NodeExecutionContextImpl

> **Parallelism**: Can run in parallel with **3A**. Prerequisite for **3C** and **3D**.

## Prerequisites
- **Phase 1** complete (`NodeBase`, `INodeExecutionContext`, `INodeRuntimeStorage`, `ExecutionSocket`, `StreamMode`)
- **Phase 2** complete (`NodeDefinition` with `NodeType`, `InlineExecutor`, `StreamSockets`)

## Can run in parallel with
- **3A** (Planner Simplification)
...
internal sealed class ExecutionRuntime
{
    ...
    internal NodeBase? GetOrCreateInstance(string nodeId) { /* ... see Phase 3 doc ... */ }
    internal async Task ExecuteNodeByIdAsync(string nodeId) { /* ... see Phase 3 doc ... */ }
...
> Full implementation code is in the original [03-phase3-execution-engine.md](../03-phase3-execution-engine.md) sections 3.1 and 3.2.
