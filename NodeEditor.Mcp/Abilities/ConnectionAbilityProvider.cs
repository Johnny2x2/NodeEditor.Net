...
public sealed class ConnectionAbilityProvider : IAbilityProvider
{
    ...
    private AbilityResult AddConnection(JsonElement p)
    {
        ...
        var outSocketVm = outNode.Outputs.FirstOrDefault(s => s.Data.Name == outSocket);
        if (outSocketVm is null)
        ...
        // Auto-detect execution vs data from the output socket type
        var isExec = p.TryGetProperty("isExecution", out var execEl)
            ? execEl.GetBoolean()
            : outSocketVm.Data.IsExecution;
